//canais,tempo_aquisicao,tempo_uart
#include <Arduino.h>
#include <EEPROM.h>
#include <stddef.h>
...
// Union Corrigida: Todos os membros alinhados em 16 bits (18 bytes totais)
typedef union {
    int16_t i16_canais[9];
    struct {
        uint16_t u16_timestamp; // Modificado para uint16_t (2 bytes) para nao corromper a0
        int16_t i16_a0;
        int16_t i16_a1;
        int16_t i16_a2;
        int16_t i16_a3;
...
// ============================================================
// Visualização de Dados (Buffer Circular)
// ============================================================

void print_data_2(st_channel_t *st_dado, uint8_t u8_num_logs, uint8_t u8_canais, uint8_t u8_indice_inicial) {
    for (uint8_t u8_k = 0; u8_k < u8_num_logs; u8_k++) {
        uint8_t u8_i = (u8_indice_inicial + u8_k) % u8_num_logs; // Inicia do dado mais antigo
...
// ============================================================
// Leitura e Processamento de Periféricos
// ============================================================

void ler_canais(st_channel_t &canal, uint8_t u8_num_canais) {
    canal.st_adc_t.u16_timestamp = (uint16_t)(millis() / 1000);

    for (uint8_t u8_i = 0; u8_i < u8_num_canais && u8_i < 8; u8_i++) {
        canal.i16_canais[u8_i + 1] = analogRead(u8_i);
    }
}
...
void verificarComandoUART() {
    while (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
            ...
            u8_idx_uart = 0; // Reseta índice para a próxima recepção

            bool formato_ok = (n == 3) && ((size_t)pos == strlen(buf_uart));
            bool limites_ok = formato_ok && (v1 >= 1 && v1 <= 8) && (v2 >= 1 && v2 <= 60) && (v3 >= 1 && v3 <= 60);
            ...
            if (limites_ok) {
                ...
                Serial.println("Config atualizada e salva na EEPROM!");
            } 
            else if (!formato_ok) {
                Serial.println("Formato invalido. Use: canais,aquisicao,uart");
            } 
            else {
                Serial.println("Fora dos limites: canais 1-8, tempos 1-60s");
            }
        } 
        else {
...
void setup() {
    ...
    if (!carregarConfig(cfg)) {
        delay(500);
        Serial.println("EEPROM corrompida/vazia! Carregando padroes.");
    ...
    Serial.print("N Canais: "); Serial.println(cfg.u16_num_canais);
    Serial.print("Tempo Aquisicao (s): "); Serial.println(cfg.u16_tempo_aquisicao);
    Serial.print("Tempo UART (s): "); Serial.println(cfg.u16_tempo_uart);
}
...
void loop() {
    ...
    if ((u32_tempo_atual - u32_tempo_aquisicao) >= (cfg.u16_tempo_aquisicao * 1000UL)) {
        ...
        u8_indice_log = (u8_indice_log + 1) % 5; // Incremento circular seguro
    }

    // Temporizador de Transferência UART
    if ((u32_tempo_atual - u32_tempo_uart) >= (cfg.u16_tempo_uart * 1000UL)) {
        u32_tempo_uart = u32_tempo_atual;
        print_data_2(st_log_canais, 5, cfg.u16_num_canais, u8_indice_log);
    }
}